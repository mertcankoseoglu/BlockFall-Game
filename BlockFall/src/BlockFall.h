...
public:
    ...
    vector<vector<bool>> rotate_left(const vector<vector<bool>> &shape_of_block);  // rotate the block to counterclockwise
    vector<vector<bool>> rotate_right(const vector<vector<bool>> &shape_of_block); // rotate the block to clockwise
};
